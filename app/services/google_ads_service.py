...
class GoogleAdsService:
    ...
    def upload_offline_conversion(self, raw_lead, conversion_type):
        ...
        try:
            ...
            conversion_action_id = self.config.conversion_action_ids.get(
    ...
    def upload_offline_conversion_adjustment(self, conversion_type, lead_id):
        """Uploads an RETRACTION conversion adjustment to Google Ads using the lead id.

        Args:
            conversion_type(ConversionType): an enum that indicates the corresponding conversion action.
            lead_id(int): a Kommo CRM lead id to construct order id.

        Returns:
            UploadConversionAdjustmentsResponse: Response from Google Ads API.

        Raises:
            GoogleAdsException: If the conversion adjustment upload fails due to a Google Ads API error.
        """
        logger.info("Received lead id %s", lead_id)
        try:
            ...
            conversion_adjustment.adjustment_date_time = self._format_time(
                datetime.now(timezone.utc).timestamp()
            )

            conversion_adjustment.order_id = f"order_{lead_id}"

            conversion_adjustment_upload_service = client.get_service(
                "ConversionAdjustmentUploadService"
            )
            conversion_adjustment_request = client.get_type(
                "UploadConversionAdjustmentsRequest"
            )

            conversion_adjustment_request.customer_id = client_customer_id
            conversion_adjustment_request.conversion_adjustments.append(
                conversion_adjustment
            )
            conversion_adjustment_request.partial_failure = True

            res = conversion_adjustment_upload_service.upload_conversion_adjustments(
                request=conversion_adjustment_request
            )
    ...
    def _create_click_conversion(self, client, raw_lead, conversion_type):
        ...
        click_conversion.conversion_date_time = self._format_time(
            raw_lead.get(
                "conversion_date_time", datetime.now(timezone.utc).timestamp()
            )
        )
        click_conversion.conversion_value = float(
            raw_lead.get(
                "conversion_value", conversion_type.default_conversion_value
            )
        )
        ...
        click_conversion.consent.ad_personalization = (
            client.enums.ConsentStatusEnum.GRANTED
        )
    ...
    def _add_user_identifiers(self, client, raw_lead, click_conversion):
        ...
        if raw_lead.get("email"):
            ...
            email_identifier.hashed_email = (
                self._normalize_and_hash_email_address(
        ...
        if raw_lead.get("phone"):
            ...
            phone_identifier.user_identifier_source = (
                client.enums.UserIdentifierSourceEnum.FIRST_PARTY
            )
